package com.platform.universitygovernance.programpath;
...
@ActiveProfiles("test")
class ProgramPathControllerIntegrationTest {
    ...
    @Test
    void rootCanCreateListGetUpdateAndDeleteProgramPaths() throws Exception {
        ...
        HttpResponse<String> createResponse = postJson(
            "/api/v1/establishments/" + firstEstablishment.getId() + "/program-paths",
            rootToken,
        ...
        String programPathId = created.get("id").asText();
        assertThat(created.get("name").asText()).isEqualTo("Excellence Program");
        ...
        HttpResponse<String> getResponse = get("/api/v1/program-paths/" + programPathId, rootToken);
        assertThat(getResponse.statusCode()).isEqualTo(200);
        assertThat(objectMapper.readTree(getResponse.body()).get("name").asText())
            .isEqualTo("Excellence Program");
        ...
        assertThat(objectMapper.readTree(updateResponse.body()).get("name").asText())
            .isEqualTo("Regular Program");

        HttpResponse<String> deleteResponse = delete("/api/v1/program-paths/" + programPathId, rootToken);
        assertThat(deleteResponse.statusCode()).isEqualTo(200);
        assertThat(objectMapper.readTree(deleteResponse.body()).get("success").asBoolean()).isTrue();
        assertThat(programPathRepository.findById(UUID.fromString(programPathId))).isEmpty();
    }

    @Test
    void adminNeedsSpecificPermissionsAndCannotCrossEstablishments() throws Exception {
        String rootToken = token(
            AccountRoleType.ROOT_SUPER_ADMIN,
            UUID.randomUUID(),
            null,
        ...
        String ownPathId = createProgramPath(rootToken, firstEstablishment.getId(), "Excellence Program");
        String otherPathId = createProgramPath(rootToken, secondEstablishment.getId(), "Regular Program");
        ...
        Permission updatePermission = permissionRepository
            .findByCodeIn(Set.of(PermissionCode.PROGRAM_PATH_UPDATE))
            .get(0);
        ...
        assertThat(get("/api/v1/program-paths/" + ownPathId, adminToken).statusCode()).isEqualTo(403);
        assertThat(patchJson(
            "/api/v1/program-paths/" + ownPathId,
            adminToken,
            "{\"name\":\"Updated Excellence Program\"}"
        ).statusCode()).isEqualTo(200);
        assertThat(delete("/api/v1/program-paths/" + ownPathId, adminToken).statusCode())
            .isEqualTo(403);
        assertThat(patchJson(
            "/api/v1/program-paths/" + otherPathId,
            adminToken,
            "{\"name\":\"Denied Update\"}"
        ).statusCode()).isEqualTo(403);
    }
    ...
    private String createProgramPath(String token, UUID establishmentId, String name) throws Exception {
