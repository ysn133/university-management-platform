...
import { saveAllSessionGradesPdf, saveStudentSessionGradesPdf, type GradeDocumentContext } from "../utils/save-student-session-grades-pdf";
...
export function SessionGradesByStudent({ context, moduleValidationThreshold, modules, session, studentDetailsPath }: Props) {
  ...
  async function downloadAll() {
    setExporting("all");
    try { await saveAllSessionGradesPdf(modules, session, context, moduleValidationThreshold); } finally { setExporting(""); }
  }

  return <div className="final-result-matrix-wrap session-grade-matrix-wrap">
    <div className="final-result-matrix-heading"><div><span>{session} session</span><strong>Grades by student</strong></div><div className="progression-header__actions"><span className="session-grade-count">{visibleStudents.length} of {students.length} students</span><button className="record-open-link progression-download-link" disabled={!students.length || Boolean(exporting)} onClick={downloadAll} type="button">{exporting === "all" ? "Preparing..." : "Download all"}</button></div></div>
