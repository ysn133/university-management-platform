...
export function ProfessorAcademicWorkspace({ establishmentId, professorId, section }: ProfessorAcademicWorkspaceProps) {
  ...
  const allProfessorAssignments = (assignmentsQuery.data ?? []).filter((assignment) => assignment.professorId === professorId && assignment.status === "ACTIVE");
  const yearAssignments = allProfessorAssignments.filter((assignment) => assignment.academicYearId === academicYearId);
  const availableSemesters = Array.from(
    yearAssignments.reduce((semesters, assignment) => {
      if (!semesters.has(assignment.semesterId)) semesters.set(assignment.semesterId, {
        id: assignment.semesterId,
        name: assignment.semesterName,
        status: assignment.semesterLifecycleStatus,
      });
      return semesters;
    }, new Map<string, { id: string; name: string; status: "PLANNED" | "ACTIVE" | "FINISHED" }>()).values(),
  ).sort((left, right) => left.name.localeCompare(right.name, undefined, { numeric: true }));
  const professorAssignments = yearAssignments.filter((assignment) => assignment.semesterId === semesterId);
  ...
  const relevantSchedules = (schedulesQuery.data ?? []).filter((schedule) => schedule.academicYearId === academicYearId && schedule.semesterId === semesterId);
  ...
  useEffect(() => {
    if (!academicYearId || !assignmentsQuery.data) return;
    const semesterOptions = Array.from(
      new Map(
        allProfessorAssignments
          .filter((assignment) => assignment.academicYearId === academicYearId)
          .map((assignment) => [assignment.semesterId, assignment]),
      ).values(),
    );
    if (semesterOptions.some((assignment) => assignment.semesterId === semesterId)) return;
    setSemesterId(
      semesterOptions.find((assignment) => assignment.semesterLifecycleStatus === "ACTIVE")?.semesterId
        ?? semesterOptions[0]?.semesterId
        ?? "",
    );
  }, [academicYearId, allProfessorAssignments, assignmentsQuery.data, semesterId]);
