...
export function ProgramFilieresPage() {
  ...
  const { programPathId: routeProgramPathId, academicYearId: routeAcademicYearId, departmentId: routeDepartmentId, degreeCycleId: routeDegreeCycleId } = useParams<{ programPathId?: string; academicYearId?: string; departmentId?: string; degreeCycleId?: string }>();
  const [searchParams, setSearchParams] = useSearchParams();
  const queryClient = useQueryClient();
  const [departmentId, setDepartmentId] = useState(() => routeDepartmentId ?? "");
  const [programPathId, setProgramPathId] = useState(() => routeProgramPathId ?? searchParams.get("programPathId") ?? "");
  const [degreeCycleId, setDegreeCycleId] = useState(() => routeDegreeCycleId ?? "");
  ...
  useEffect(() => { setDepartmentId(routeDepartmentId ?? ""); }, [routeDepartmentId]);
  useEffect(() => { setDegreeCycleId(routeDegreeCycleId ?? ""); }, [routeDegreeCycleId]);
  ...
  const isDepartmentScoped = Boolean(routeDepartmentId);
  const isCycleScoped = Boolean(routeDegreeCycleId);
  ...
  const programPathFor = (programId: string) => isYearScoped && routeAcademicYearId && routeProgramPathId
    ...
    : isDepartmentScoped && routeDepartmentId
    ? `${workspacePath}/departments/${routeDepartmentId}/programs/${programId}`
    : isCycleScoped && routeDegreeCycleId
    ? `${workspacePath}/degree-cycles/${routeDegreeCycleId}/programs/${programId}`
  ...
  return <div className="management-page academic-directory-page program-directory-page">
    ...
    {isDepartmentScoped && <Link className="context-back-link" to={`${workspacePath}/departments`}>← Back to departments</Link>}
    {isCycleScoped && <Link className="context-back-link" to={`${workspacePath}/degree-cycles`}>← Back to degree cycles</Link>}
    <header className={isYearScoped ? "academic-year-section-header" : "management-page-header management-page-header--compact"}><div><p className="management-kicker">{isYearScoped ? "Selected program path" : isPathScoped ? "Program path" : isDepartmentScoped ? "Department" : isCycleScoped ? "Degree cycle" : "Academic structure"}</p><h1>{selectedPath?.name ?? selectedDepartment?.name ?? (isCycleScoped ? cycleNames.get(degreeCycleId) : undefined) ?? "Programs / Filières"}</h1><p>{selectedPath ? `Programs and filières classified under the ${selectedPath.name} path${selectedAcademicYear ? ` for ${selectedAcademicYear.label}` : ""}.` : isDepartmentScoped ? `Programs and filières offered by ${selectedDepartment?.name ?? "this department"}.` : isCycleScoped ? `Programs and filières framed by ${cycleNames.get(degreeCycleId) ?? "this degree cycle"}.` : "Define each department's programs and classify them by degree cycle and study path."}</p></div><button className="management-primary-button" disabled={!canCreate} onClick={openCreate} type="button">New Program</button></header>
    {catalogError && <div className="management-alert management-alert--error">{errorMessage(catalogError)}</div>}
    <section className="directory-toolbar program-directory-toolbar">{!isDepartmentScoped && <label><span>Department</span><select disabled={catalogsPending || !departmentsQuery.data?.length} onChange={(event) => setDepartmentId(event.target.value)} value={departmentId}><option value="">All departments</option>{departmentsQuery.data?.map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</select></label>}{!isPathScoped && !isYearScoped && <label><span>Program path</span><select disabled={pathsQuery.isPending || !pathsQuery.data?.length} onChange={(event) => updateProgramPathFilter(event.target.value)} value={programPathId}><option value="">All paths</option>{pathsQuery.data?.map((path) => <option key={path.id} value={path.id}>{path.name}</option>)}</select></label>}{!isCycleScoped && <label><span>Degree cycle</span><select disabled={cyclesQuery.isPending || !cyclesQuery.data?.length} onChange={(event) => setDegreeCycleId(event.target.value)} value={degreeCycleId}><option value="">All cycles</option>{cyclesQuery.data?.map((cycle) => <option key={cycle.id} value={cycle.id}>{cycle.name}</option>)}</select></label>}<label className="search-field"><span>Search</span><input onChange={(event) => setSearch(event.target.value)} placeholder="Program name or code" value={search} /></label><span className="directory-result-count">{programs.length} {programs.length === 1 ? "program" : "programs"}</span></section>
