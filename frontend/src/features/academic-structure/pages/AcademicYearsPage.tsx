...
export function AcademicYearsPage() {
  ...
  return <div className="management-page academic-directory-page">
    ...
    <section className="management-panel directory-panel">{yearsQuery.isPending ? <div className="panel-empty">Loading academic years...</div> : yearsQuery.isError ? <div className="panel-empty panel-empty--error">{errorMessage(yearsQuery.error)}</div> : years.length === 0 ? <div className="panel-empty"><strong>No academic year found.</strong><p>Create the first annual academic period.</p></div> : <div className="resource-table-wrapper"><table className="resource-table academic-resource-table"><thead><tr><th>Academic year</th><th>Period</th><th>Status</th><th>Actions</th></tr></thead><tbody>{years.map((year) => <tr key={year.id}><td><Link className="resource-name resource-name--link" to={`${workspacePath}/academic-years/${year.id}/program-paths`}><span className="resource-monogram">{String(year.startYear).slice(2)}</span><strong>{year.label}</strong></Link></td><td>{year.startYear} to {year.endYear}</td><td><StatusBadge status={year.status} /></td><td><div className="row-actions"><Link className="record-open-link" to={`${workspacePath}/academic-years/${year.id}/program-paths`}>Open paths</Link><button onClick={() => openEdit(year)} type="button">Edit</button><button className="danger-text" onClick={() => setDeleting(year)} type="button">Delete</button></div></td></tr>)}</tbody></table></div>}</section>
