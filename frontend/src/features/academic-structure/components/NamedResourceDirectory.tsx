...
interface NamedResourceDirectoryProps {
  ...
  resourcePath?: (resource: NamedResource) => string;
}
...
export function NamedResourceDirectory({ title, singular, description, emptyDescription, queryKey, load, create, update, remove, resourcePath }: NamedResourceDirectoryProps) {
  ...
  return <div className="management-page academic-directory-page">
    ...
    <section className="management-panel directory-panel">
      {resourcesQuery.isPending ? <div className="panel-empty">Loading {title.toLowerCase()}...</div> : resourcesQuery.isError ? <div className="panel-empty panel-empty--error">{errorMessage(resourcesQuery.error)}</div> : resources.length === 0 ? <div className="panel-empty"><strong>No {singular.toLowerCase()} found.</strong><p>{search ? "Try a different search term." : emptyDescription}</p></div> : <div className="resource-table-wrapper"><table className="resource-table academic-resource-table"><thead><tr><th>{singular}</th><th>Last updated</th><th>Actions</th></tr></thead><tbody>{resources.map((resource) => <tr key={resource.id}><td>{resourcePath ? <Link className="resource-name resource-name--link" to={resourcePath(resource)}><span className="resource-monogram">{resource.name.slice(0, 2).toUpperCase()}</span><strong>{resource.name}</strong></Link> : <div className="resource-name"><span className="resource-monogram">{resource.name.slice(0, 2).toUpperCase()}</span><strong>{resource.name}</strong></div>}</td><td>{resource.updatedAt ? new Date(resource.updatedAt).toLocaleDateString() : "Not available"}</td><td><div className="row-actions">{resourcePath && <Link className="record-open-link" to={resourcePath(resource)}>Open programs</Link>}<button onClick={() => { setEditing(resource); setName(resource.name); }} type="button">Edit</button><button className="danger-text" onClick={() => setDeleting(resource)} type="button">Delete</button></div></td></tr>)}</tbody></table></div>}
    </section>
