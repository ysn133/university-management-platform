...
export function StudentGradesPage() {
  ...
  const [resultView, setResultView] = useState<"EFFECTIVE" | "ORIGINAL">("EFFECTIVE");
  const gradesQuery = useQuery({ queryKey: studentOverviewKeys.grades(resultView), queryFn: () => getMyStudentGrades(resultView) });
  ...
  const hasRevisedResults = resultView === "EFFECTIVE" && contextGrades.some((grade) => grade.revised);
  ...
  return <div className="management-page student-grades-page">
    ...
    <section className="management-panel student-grades-panel">
      <div className="student-grades-toolbar">
        <div className="student-workspace-tabs" role="tablist"><button aria-selected={view === "NORMAL"} onClick={() => setView("NORMAL")} role="tab" type="button">Normal</button><button aria-selected={view === "RATTRAPAGE"} onClick={() => setView("RATTRAPAGE")} role="tab" type="button">Rattrapage</button><button aria-selected={view === "FINAL"} onClick={() => setView("FINAL")} role="tab" type="button">Final</button><button aria-selected={view === "ACADEMIC_YEAR"} onClick={() => setView("ACADEMIC_YEAR")} role="tab" type="button">Academic year</button></div>
        <div className="student-grades-toolbar__context"><div className="student-grades-selectors"><label><span>Academic year</span><select disabled={contextsQuery.isPending || !years.length} onChange={(event) => { setAcademicYearId(event.target.value); setSemesterId(""); }} value={academicYearId}>{years.map((year) => <option key={year.id} value={year.id}>{year.label}</option>)}</select></label>{view !== "ACADEMIC_YEAR" && <label><span>Semester</span><select disabled={!semesters.length} onChange={(event) => setSemesterId(event.target.value)} value={semesterId}>{semesters.map((semester) => <option key={semester.semesterRegistrationId} value={semester.semesterId}>{semester.semesterName}</option>)}</select></label>}</div>{view !== "ACADEMIC_YEAR" && (hasRevisedResults || resultView === "ORIGINAL") && <button className="student-result-view-toggle" onClick={() => setResultView(resultView === "EFFECTIVE" ? "ORIGINAL" : "EFFECTIVE")} type="button">{resultView === "EFFECTIVE" ? "Original result" : "Updated result"}</button>}</div>
      </div>

      {view !== "ACADEMIC_YEAR" && <header className="student-grades-context"><div className="student-grades-context__identity"><span>{view === "NORMAL" ? "Normal session" : view === "RATTRAPAGE" ? "Rattrapage session" : "Final module results"}</span><strong>{selectedContext?.programFiliereName ?? "Academic results"}</strong><small>{selectedContext ? `${selectedContext.academicLevelName} · ${selectedContext.semesterName} · ${selectedContext.academicYearLabel}` : "Select an academic context"}</small></div>{view === "FINAL" && <div className="student-grades-context__summary"><span>Average</span><strong>{average?.toFixed(2) ?? "—"}</strong><small>{visibleGrades.length} module{visibleGrades.length === 1 ? "" : "s"}</small></div>}</header>}

      {view === "ACADEMIC_YEAR" ? <StudentAcademicYearResult context={selectedYearContext} graduation={graduation} loading={contextsQuery.isPending || progressionQuery.isPending || graduationQuery.isPending} progression={progression} /> : gradesQuery.isPending || contextsQuery.isPending ? <div className="panel-empty">Loading results...</div> : !selectedContext ? <div className="panel-empty"><strong>No academic registration is available.</strong></div> : visibleGrades.length === 0 ? <div className="panel-empty"><strong>No published {view === "NORMAL" ? "Normal Session" : view === "RATTRAPAGE" ? "Rattrapage" : "Final"} result for this semester.</strong></div> : <div className="student-grades-table-wrap"><table className="student-grades-table"><thead><tr><th>Module</th><th>Session</th><th>Inscription</th><th>Grade</th>{view === "FINAL" && <th>Result</th>}</tr></thead><tbody>{visibleGrades.map((grade) => <tr key={`${view}:${grade.gradeRecordId}:${grade.academicYearId}:${grade.semesterId}`}><td><span>{grade.subjectModuleCode}</span><strong>{grade.subjectModuleTitle}</strong></td><td>{view === "FINAL" ? "Final" : grade.sessionType === "NORMAL" ? "Normal" : "Rattrapage"}</td><td>{grade.inscriptionNumber === 1 ? <span className="student-inscription-label">First</span> : <span className="second-inscription-badge">2nd inscription</span>}</td><td><GradeValue final={view === "FINAL"} grade={grade} /></td>{view === "FINAL" && <td><span className={`student-result-status student-result-status--${grade.moduleResultStatus?.toLowerCase() ?? "pending"}`}>{grade.moduleResultStatus ? resultLabels[grade.moduleResultStatus] : "Pending"}</span></td>}</tr>)}</tbody></table></div>}
    </section>
  </div>;
}
