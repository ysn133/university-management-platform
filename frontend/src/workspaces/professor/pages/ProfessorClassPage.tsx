...
export function ProfessorClassPage() {
  ...
  const initialTab = requestedTab === "grades" || requestedTab === "attendance" ? requestedTab : "schedule";
  const initialGradeView = searchParams.get("session");
  const [activeTab, setActiveTab] = useState<"schedule" | "students" | "attendance" | "grades" | "exams">(initialTab);
  const [requestedGradeExamId, setRequestedGradeExamId] = useState(searchParams.get("examId") ?? "");
  const [gradeView, setGradeView] = useState<"NORMAL" | "RATTRAPAGE" | "FINAL">(initialGradeView === "RATTRAPAGE" ? "RATTRAPAGE" : initialGradeView === "FINAL" ? "FINAL" : "NORMAL");
  ...
  const returnPath = returnContext === "grades" ? "/professor/grades" : returnContext === "exams" ? "/professor/exams" : returnContext === "attendance" ? "/professor/attendance" : `/professor/modules/${subjectModuleId}`;
  const returnLabel = returnContext === "grades" ? "Back to Grades" : returnContext === "exams" ? "Back to Exam Schedule" : returnContext === "attendance" ? "Back to Attendance" : "Back to module classes";

  return <div className="management-page professor-class-page">
    <Link className="management-back-link" to={returnPath}>← {returnLabel}</Link>
    ...
    {loading ? <div className="management-panel panel-empty">Loading class...</div> : !responsibility ? <div className="management-panel panel-empty"><strong>Class not found.</strong><p>This class is not assigned to your account for this module.</p></div> : <>
      ...
      <nav aria-label="Class workspace" className="curriculum-section-tabs" role="tablist"><button aria-selected={activeTab === "schedule"} onClick={() => setActiveTab("schedule")} role="tab" type="button">Schedule</button><button aria-selected={activeTab === "students"} onClick={() => setActiveTab("students")} role="tab" type="button">Students</button><button aria-selected={activeTab === "attendance"} onClick={() => setActiveTab("attendance")} role="tab" type="button">Attendance</button><button aria-selected={activeTab === "grades"} onClick={() => setActiveTab("grades")} role="tab" type="button">Grades</button><button aria-selected={activeTab === "exams"} onClick={() => setActiveTab("exams")} role="tab" type="button">Exam Schedule</button></nav>
