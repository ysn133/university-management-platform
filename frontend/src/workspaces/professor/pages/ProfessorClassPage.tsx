...
export function ProfessorClassPage() {
  ...
  const selectedGradeExam = classExams.find((exam) => exam.id === requestedGradeExamId) ?? classExams.find((exam) => exam.sessionType === "NORMAL") ?? classExams[0];
  ...
  return <div className="management-page professor-class-page">
    ...
    {loading ? <div className="management-panel panel-empty">Loading class...</div> : !responsibility ? <div className="management-panel panel-empty"><strong>Class not found.</strong><p>This class is not assigned to your account for this module.</p></div> : <>
      ...
      : activeTab === "grades" ? <section className="management-panel professor-class-workspace professor-class-grades"><header className="panel-header panel-header--bordered"><div><p className="management-kicker">Assessment</p><h2>Grade Sheet</h2><p>Enter all student results for one examination session.</p></div>{classExams.length > 1 && <label className="professor-grade-exam-select"><span>Exam session</span><select onChange={(event) => { setRequestedGradeExamId(event.target.value); setSearchParams({ tab: "grades", examId: event.target.value }); }} value={selectedGradeExam?.id}>{classExams.map((exam) => <option key={exam.id} value={exam.id}>{exam.sessionType === "NORMAL" ? "Normal" : "Rattrapage"} · {exam.examDate}</option>)}</select></label>}</header>{examsQuery.isPending ? <div className="panel-empty">Loading exams...</div> : !selectedGradeExam ? <div className="panel-empty"><strong>No published exam available for grading.</strong></div> : <ProfessorGradeSheet exam={selectedGradeExam} />}</section>
